...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($stmt_account->execute()) {
        ...
        if ($stmt_owner->execute()) {
            ...
            if (!empty($_FILES['application_letter']['name'])) {
                $files = $_FILES;
                ...
                // Process each file
                $file_fields = ['application_letter', 'owner_photo', 'business_permit', 'tax_certificate', 'dti_certificate', 'clearance', 'bir_certificate', 'medical_certificate'];
                foreach ($file_fields as $file_field) {
                    if (!empty($files[$file_field]['name'])) {
                        $file_name = $files[$file_field]['name'];
                        $file_tmp = $files[$file_field]['tmp_name'];
                        $file_type = $files[$file_field]['type'];
                        $file_size = $files[$file_field]['size'];
                        ...
                        if (move_uploaded_file($file_tmp, $file_path)) {
                            ...
                            $_SESSION['error_message'] = "Uploading file '$file_name' failed.";
                        }
                    }
                }
            }
...
header("Location: ../signup_accred.php");
exit;
$conn->close();
?>
