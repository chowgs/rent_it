...
<head>
    ...
    <link rel="stylesheet" href="sa-css/message.css">
                ...
                <h6 class="sub-dash" style="font-weight: 600;">Dashboard / <span style="font-weight: 100;">Chats</span></h6>
                    <div class="row">
                        ...
                        if (isset($_SESSION['AccountID'])) {
                            ...
                            $senderFullName = getFullName($conn, $accountID, 'admin');

                                // Fetch unique user IDs based on sent and received messages
                                $query = "SELECT DISTINCT userID FROM (
                                    SELECT SenderID AS userID, Timestamp FROM messages WHERE ReceiverID = ?
                                    UNION
                                    SELECT ReceiverID AS userID, Timestamp FROM messages WHERE SenderID = ?
                                ...
                                <div class="col-md-4 lists">
                                    ...
                                    if ($result->num_rows > 0) {
                                        while ($row = $result->fetch_assoc()) {
                                            ...
                                            echo '<div class="receiverName" data-receiver-id="' . $senderID . '" data-receiver-name="' . htmlspecialchars($senderFullName) . '">' . htmlspecialchars($senderFullName) . '</div>';
                                        }
                                    } else {
