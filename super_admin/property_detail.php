                ...
                if (isset($_GET['id'])) {
                    ...
                    if ($properties) {
                        foreach ($properties as $property) {
                            ...
                            // Fetch the OwnerID using AccountID
                            $acc = "SELECT AdminID FROM admin WHERE AccountID = ?";
                            ...
                            echo '<p style="margin-top:50px;">You can contact me through <br> Contact #: '.$property['ContNum'].'<br> Social Media: <a href="'.$property['fblink'].'" target="_blank">'.$property['fblink'].'</a></p><br>';
                            echo '</div>';
                            echo '</div>';
                            echo '</div>';
                            echo '</div>';
    ...
    <script>
        ...
        $(document).ready(function(){
            $('.view-rooms-btn').on('click', function(){
                ...
                $.ajax({
                    url: 'process/fetch_rooms_prop.php',
                    type: 'POST',
                    data: {id: propertyID},
                    success: function(response){
